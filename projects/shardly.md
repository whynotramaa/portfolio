...
- Problem: Full-text search engines are hard to learn from or defend in an interview when they hide behind an embedded DB or a search library.
- Solution: A small self-contained document store with full-text search written from scratch in TypeScript. Append-only segments, write-ahead log, hand-rolled tokenizer, inverted index, and BM25 ranker with per-term score explanations, plus a Next.js workspace for ingest/search/benchmark. See `ENGINEERING.md` in the repo for the design arguments and limits.
- Tech: TypeScript, Node 20+, Fastify + @fastify/cors + @fastify/multipart, unpdf (PDF ingestion), Next.js frontend, Vitest + tsx, custom storage engine (segments + WAL + snapshots + group commit + compaction) and BM25 (k1=1.5, b=0.75). Recorded 50k-doc benchmark: indexed queries ~6–21 ms vs naive ~750–840 ms with top-hit agreement.
- Link: https://github.com/whynotramaa/shardly
- Website: none (local API defaults to http://localhost:3001)

---

> Original README from https://github.com/whynotramaa/shardly below, kept verbatim.
